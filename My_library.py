...
###########################################################################################
# 輸出時間字串, 依指定格式 // 參數如下表, 由 0~7 // 可以直接使用, 不需要建立物件就能呼叫.
class DateTimeString:
    ...
    @classmethod
    def now(cls, index: int = 0) -> str:
        try:
            fmt = cls._FORMATS[index]
        except IndexError:
            print("%s: %d" % (libmsg.output(1), index))                 # libmsg 1, "!!! 無效的日期時間格式索引"
            raise ValueError("%s: %d" % (libmsg.output(1), index))      # libmsg 1, "!!! 無效的日期時間格式索引"
...
def rename_with_timestamp(filename: str) -> int:
    try:
        ...
        new_name = f"{base}.{DateTimeString.now(7)}{ext}"
